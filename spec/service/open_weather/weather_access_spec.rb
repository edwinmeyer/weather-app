...
RSpec.describe OpenWeather::WeatherAccess, type: :model do
  describe 'get_weather(' do
    ...
    before(:each) do
      WebMock.stub_request(:get, "https://api.openweathermap.org/data/3.0/onecall?Accept=application/json&appid=dummy%20api%20key&exclude=daily,alerts&lat=37.3230&lon=-122.03228").
        with(
          headers: {
            'Accept'=>'*/*',
      ...
      WebMock.stub_request(:get, "http://api.openweathermap.org/geo/1.0/zip?Accept=application/json&appid=dummy%20api%20key&zip='',US").
        with(
          headers: {
            'Accept'=>'*/*',
            'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
    ...
    # TODO Write specs for zipcode provided
    # This needs more work:
    xit 'returns the current weather for Cupertino, CA' do
    ...
    xit 'returns the expected cache value' do
      weather_hash = owa_instance.get_weather('', '37.3230', '-122.0322') # Cupertino, CA, 95014
